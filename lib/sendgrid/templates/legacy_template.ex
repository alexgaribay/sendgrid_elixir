defmodule SendGrid.LegacyTemplate do
  @moduledoc """
  ...
  @spec new(Map.t, :json) :: LegacyTemplate.t
  ...
  defimpl Jason.Encoder do
    def encode(%SendGrid.LegacyTemplate{} = template, opts) do
      raw = template
            |> Map.from_struct()
            |> Map.drop([:updated_at])
            |> Enum.filter(fn({_k,v}) -> v != nil end)
            |> Map.new()
            |> Map.put(:generation, :legacy)
